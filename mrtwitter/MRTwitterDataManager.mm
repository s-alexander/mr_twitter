...
@implementation MRTwitterDataManager
...
+(MRTwitterDataManager *) loadTarelkaDataManager {
  MRTwitterDataManager * dataManager = [[[self alloc]init]autorelease];
  return dataManager;
}

+(MRTwitterDataManager *) sharedMRTwitterDataManager {
  static MRTwitterDataManager * manager = [[self loadTarelkaDataManager] retain];
  return manager;
}
...
-(NSURL *) sqlDatabaseURL {
  return [[self applicationDataDirectory] URLByAppendingPathComponent:@"mrtwitter.sqlite"];
}
